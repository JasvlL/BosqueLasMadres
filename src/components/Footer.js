import Link from "next/link";

// SVGs inline para redes sociales (lucide-react v1.21 no incluye íconos de marcas)
const FacebookIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/>
  </svg>
);

const InstagramIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" xmlns="http://www.w3.org/2000/svg">
    <rect x="2" y="2" width="20" height="20" rx="5" ry="5"/>
    <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/>
    <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/>
  </svg>
);

const YoutubeIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M22.54 6.42a2.78 2.78 0 0 0-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46A2.78 2.78 0 0 0 1.46 6.42 29 29 0 0 0 1 12a29 29 0 0 0 .46 5.58A2.78 2.78 0 0 0 3.41 19.54C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 0 0 1.95-1.96A29 29 0 0 0 23 12a29 29 0 0 0-.46-5.58z"/>
    <polygon fill="white" points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02"/>
  </svg>
);

const XIcon = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
  </svg>
);

export default function Footer() {
  return (
    <footer className="main-footer">
      <div className="container">
        <div className="footer-content">
          <div className="footer-section">
            <h3>Corredor Biológico Bosque Las Madres</h3>
            <p>Dedicados a la conservación y educación ambiental</p>
            {/* Botones de redes sociales con iconos SVG profesionales */}
            <div className="social-buttons">
              <a
                href="https://facebook.com"
                className="social-btn social-btn--facebook"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Facebook"
              >
                <FacebookIcon />
              </a>
              <a
                href="https://instagram.com"
                className="social-btn social-btn--instagram"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram"
              >
                <InstagramIcon />
              </a>
              <a
                href="https://youtube.com"
                className="social-btn social-btn--youtube"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="YouTube"
              >
                <YoutubeIcon />
              </a>
              <a
                href="https://twitter.com"
                className="social-btn social-btn--twitter"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Twitter / X"
              >
                <XIcon />
              </a>
            </div>
          </div>
          <div className="footer-section">
            <h4>Enlaces Rápidos</h4>
            <ul>
              <li><Link href="/sobre-nosotros">Nosotros</Link></li>
              <li><Link href="/sobre-el-corredor">El Corredor</Link></li>
              <li><Link href="/aves">Biodiversidad</Link></li>
              <li><Link href="/iniciativas">Iniciativas</Link></li>
              <li><Link href="/conteo-aves-2024">Conteo 2024</Link></li>
              <li><Link href="/mapa">Mapa y Visita</Link></li>
              <li><Link href="/voluntariado">Voluntariado</Link></li>
            </ul>
          </div>
          <div className="footer-section" id="contacto">
            <h4>Contacto</h4>
            <p>Coordinadora: Sofia Stein</p>
            <p>
              Email:{" "}
              <a href="mailto:sofiastein1@gmail.com" className="footer-email">
                sofiastein1@gmail.com
              </a>
            </p>
            <p>Teléfono: 8723 4884</p>
          </div>
        </div>
        <div className="footer-bottom">
          <p>
            &copy; 2025 Corredor Biológico Bosque Las Madres. Todos los derechos
            reservados.
          </p>
        </div>
      </div>
    </footer>
  );
}
